...
```rust
#[allow(overflowing_literals)] // denying overflow (not good tho)
fn main() {
...
fn main() {
    ...
    assert_eq!(1000 as u8, 232); //by subtracting 256 multiple times you get 232
...
fn main() {
    ...
    let b = a as *const [u8];
    unsafe {
        assert_eq!(std::mem::size_of_val(&*b), 13) //u8 is only one byte each therefore, 1*13
    }
